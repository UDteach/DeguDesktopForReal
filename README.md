...
- **v0.7.0（2026-09-26）**：マルチモニタの表示先選択、接続・取り外しへの対応、毛色ごとのサイズ設定を追加。
...
「デグーの表示先」で、カーソルのあるモニタ、メインモニタ、特定のモニタ、すべてのモニタを選べます。「すべてのモニタ」は各画面に同じ毛色・動きを表示し、大発生モードも各画面で再生します。選択したモニタを取り外すとメインモニタへ表示し、再接続すると元のモニタへ戻ります。カーソルを基準にする場合は、登場する時点のカーソル位置で表示先が決まります。

「表示サイズ」→「全体の基本サイズ」で共通のサイズを選び、「毛色ごと」で特定の子だけサイズを変えられます。「全体の基本サイズを使う」でその子の個別設定を解除し、「個別設定を消して全体に統一」で全色を基本サイズに戻せます。ポモドーロの残り時間はメインモニタの右上に表示します。
